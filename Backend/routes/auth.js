...
router.post('/register', async (req, res) => {
    const { ime, priimek, email, geslo, leto_rojstva, opis, nivo, letna_karta, krediti } = req.body;
    ...
    try {
        ...
        const [result] = await pool.query(
            ...
            (ime, priimek, email, geslo_hash, leto_rojstva, opis, nivo, letna_karta, krediti)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [ime, priimek, email, hashedPassword, leto_rojstva || null, opis || null, nivo || 'Rekreativec', letna_karta ? 1 : 0, krediti || 0]
        );
        const user = {
            id: result.insertId,
            ime,
...
// Trenutni uporabnik (preveri token)
router.get('/me', async (req, res) => {
    ...
    try {
        ...
        const [rows] = await pool.query(
            `SELECT id, ime, priimek, email, leto_rojstva, opis, nivo, letna_karta, krediti 
