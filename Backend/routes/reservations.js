...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (conflicts.length > 0) return res.status(400).json({ message: 'Termin je že zaseden' });
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        if (reservationRows.length === 0) return res.status(404).json({ message: 'Rezervacija ne obstaja' });
        if (reservationRows[0].user_id !== userId && !isAdmin) return res.status(403).json({ message: 'Nimate dovoljenja' });
